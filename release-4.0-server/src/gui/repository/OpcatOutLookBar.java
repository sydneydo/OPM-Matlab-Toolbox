 ...
 * visible component
 */
public class OpcatOutLookBar extends JPanel implements ActionListener {
	/**
	 * 
	 */
	private static final long serialVersionUID = 2018947443276727603L;
	...
	/**
	 * Creates a new OpcatOutLookBar; after which you should make repeated calls
	 * to addBar() for each bar
	 */
	public OpcatOutLookBar(Repository rep) {
	...
	public void setBold() {
		...
		Iterator iter = bars.values().iterator() ; 
		while(iter.hasNext()) {
			BarInfo info = (BarInfo) iter.next() ; 
			JButton button = info.getButton() ; 
			Font font = new Font(button.getFont().getName(), Font.BOLD, button.getFont().getSize() ) ; 
			button.setFont(font) ; 	
			button.repaint();
		}
	}
	...
	/**
	 * Adds the specified component to the OpcatOutLookBar and sets the bar's
	 * name
	 * 
	 * @param name
	...
	/**
	 * Removes the specified bar from the OpcatOutLookBar
	 * 
	 * @param name
	...
	public void actionPerformed(ActionEvent e) {
		...
		for (Iterator i = this.bars.keySet().iterator(); i.hasNext();) {
			...
			if (barInfo.getButton() == e.getSource()) {
				...
				repo.changeView(this.visibleBar) ; 				
				render();
				return;
			}
			currentBar++;
...
//		JFrame frame = new JFrame("OpcatOutLookBar Test");
//		OpcatOutLookBar outlookBar = new OpcatOutLookBar();
