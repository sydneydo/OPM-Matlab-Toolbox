...
public class AlgoExectMatch implements AlgoInterface {
	...
	public Vector PreformSearch(Entry parent, IXSystem opcat) {
		...
		while (entEnum.hasMoreElements()) {
			...
			if (ent.getName().equals(this.options.getSearchText())
					&& this.options.inName) {
				...
				if ((ent instanceof IXStateEntry) && this.options.forStates) {
					...
					if (parent != null) {
						if (parent instanceof ObjectEntry) {
							if(state.getParentIXObjectEntry().getId() == parent.getId()) {
								found = true ; 
							}
						} else {
							found = true;
				...
				if (((ent instanceof IXLinkEntry) || (ent instanceof IXRelationEntry)) && this.options.forLinks) {
					found = true;
				}				
